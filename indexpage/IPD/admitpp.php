...
$h1="SELECT `dofadmit` FROM `patient_admit` WHERE `pid` ='$pid'";
$h2=mysql_query($h1);
$h3=mysql_fetch_assoc($h2);	
	$date1=strftime('%d-%m-%Y',strtotime($h3['dofadmit']));
//$n=date('Y-m-d');
  $date2=strftime('%d-%m-%Y',strtotime($hh));
  $diff = abs(strtotime($date1) - strtotime($date2));
  $total_days = floor ($diff /  (60*60*24)) + 1;
	
	
	$pp="SELECT * FROM `ipd_chg` WHERE `name`='$room'";
	$pp1=mysql_query($pp);
	$pp2=mysql_fetch_assoc($pp1);
	
  $pr=$pp2['price'];
	
$chg=$total_days * $pr;
 $qq="UPDATE `patient_admit` SET `bedno`='$bed',`drname`='$dr',`rtype`='$room',`tran_date`='$hh',`chg`='$chg' WHERE pid='$pid'";
 //exit;
  mysql_select_db($database_cn, $cn);
  $Result1 = mysql_query($qq, $cn) or die(mysql_error());

  $insertGoTo = "showadmitp1.php";
              ...
              <div id="basicFormExample" class="panel-collapse collapse in">
                ...
                <form action="<?php echo $editFormAction; ?>" method="POST" name="frm" >
                  <table border="1" class="table-bordered table-condensed table-green table-hover table-responsive">
                    ...
                    <tr><td>Select Doctor</td><td><select name="dctr" required >
