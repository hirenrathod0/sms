...
<script type="text/javascript" language="javascript" >
 $(document).ready(function(){
  ...
  $(document).on('click', '#insert', function(){
   var adminid = $('#data1').text();
   var adminname = $('#data2').text();
   var username = $('#data3').text();
   var password = $('#data4').text();
   var contactno = $('#data5').text();
   if(adminname != '' )
   {
    $.ajax({
     url:"insert.php",
     method:"POST",
     data:{adminid:adminid, adminname:adminname ,username:username ,password:password ,contactno:contactno},
     success:function(data)
     {
      $('#alert_message').html('<div class="alert alert-success">'+data+'</div>');
      $('#user_data').DataTable().destroy();
