...
$query_getlist = "SELECT * FROM equipmenttype ORDER BY etid DESC";
...
mysql_select_db($database_cn, $cn);
$query_getedit = sprintf("SELECT * FROM equipmenttype WHERE etid = %s", $colname_getedit);
...
if ((isset($_GET['did'])) && ($_GET['did'] != "")) {
  $deleteSQL = sprintf("DELETE FROM equipmenttype WHERE etid=%s",
                       GetSQLValueString($_GET['did'], "int"));

  mysql_select_db($database_cn, $cn);
  $Result1 = mysql_query($deleteSQL, $cn) or die(mysql_error());

  $deleteGoTo = "equipment_type.php?msg=d";
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "fcat")) {
  ...
  if($_POST['editid']){
  $gg=$_POST['editid'];
    $insertSQL = sprintf("update equipmenttype set name=%s where etid='$gg'",
                       GetSQLValueString($_POST['name'], "text"));

  mysql_select_db($database_cn, $cn);
  $Result1 = mysql_query($insertSQL, $cn) or die(mysql_error());

  $insertGoTo = "equipment_type.php?msg=e";
   header(sprintf("Location: %s", $insertGoTo));
  }
  else
  {
 
   $insertSQL = sprintf("INSERT INTO equipmenttype (name) VALUES (%s)",
                       GetSQLValueString($_POST['name'], "text"));

  mysql_select_db($database_cn, $cn);
  $Result1 = mysql_query($insertSQL, $cn) or die(mysql_error());

  $insertGoTo = "equipment_type.php?msg=s";
   header(sprintf("Location: %s", $insertGoTo));
...
if( confirm('are you sure you want to delete?'))
{
 ...
 window.location='equipment_type.php?did='+id;
      ...
      <div class="col-lg-12">
        <div class="page-title">
         <ol class="breadcrumb">
            ...
            <li class="active">Equipment Type</li>
          </ol>
        </div>
      </div>
      <!-- /.col-lg-12 -->
                ...
                <div class="portlet-body">
                  <form role="form" name="fcat" action="<?php echo $editFormAction; ?>" method="POST">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="exampleInputEmail1" placeholder="Enter Equipment Name " onblur="makeupper(this.id);" required name="name" value="<?php echo $row_getedit['name']; ?>">
                    </div>
					<input type="hidden" value="<?php echo $row_getedit['etid']; ?>" name="editid" />
										 ...
										 <?php $k=0; do { ?>
                                            <tr>
                                                <td ><?php echo $k=$k+1; ?></td>
												 ...
												 <td>
																			 <a href="equipment_type.php?eid=<?php echo $row_getlist['etid']; ?>" class="btn btn-info"><i class="fa fa-edit"> </i></a>
												  <button  type="button" class="btn btn-danger"	 onclick="getc(<?php echo $row_getlist['etid']; ?>)" ><i class="fa fa-times"></i></button>
