...
$query_getlist = "SELECT * FROM category ORDER BY cid DESC";
...
mysql_select_db($database_cn, $cn);
$query_getedit = sprintf("SELECT * FROM category WHERE cid = %s", $colname_getedit);
...
if ((isset($_GET['did'])) && ($_GET['did'] != "")) {
  ...
  $deleteGoTo = "room_category.php?msg=d";
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "fcat")) {
  ...
  $gg=$_POST['editid'];
    $insertSQL = sprintf("update category set name=%s where cid='$gg'",
                       GetSQLValueString($_POST['cname'], "text"));

  mysql_select_db($database_cn, $cn);
  $Result1 = mysql_query($insertSQL, $cn) or die(mysql_error());

  $insertGoTo = "room_category.php?msg=e";
  ...
  {
  $insertSQL = sprintf("INSERT INTO category (name) VALUES (%s)",
                       GetSQLValueString($_POST['cname'], "text"));

  mysql_select_db($database_cn, $cn);
  $Result1 = mysql_query($insertSQL, $cn) or die(mysql_error());

  $insertGoTo = "room_category.php?msg=s";
  header(sprintf("Location: %s", $insertGoTo));
  }
...
if( confirm('are you sure you want to delete?'))
{
 ...
 window.location='room_category.php?did='+id;
                ...
                <div class="portlet-body">
                  <form role="form" name="fcat" action="<?php echo $editFormAction; ?>" method="POST">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="exampleInputEmail1" onblur="makeupper(this.id);" placeholder="Enter Category Name " required name="cname" value="<?php echo $row_getedit['name']; ?>">
                    </div>
					<input type="hidden" value="<?php echo $row_getedit['cid']; ?>" name="editid" />
										 ...
										 <?php $k=0; do { ?>
                                            <tr>
                                                <td ><?php echo $k=$k+1; ?></td>
												 ...
												 <td>
																			 <a href="room_category.php?eid=<?php echo $row_getlist['cid']; ?>" class="btn btn-info"><i class="fa fa-edit"> </i></a>
												  <button  type="button" class="btn btn-danger"	 onclick="getc(<?php echo $row_getlist['cid']; ?>)" ><i class="fa fa-times"></i></button>
