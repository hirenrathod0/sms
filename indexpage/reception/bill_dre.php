...
$colname_rs_dre = "-1";
if (isset($_GET['id'])) {
  $colname_rs_dre = $_GET['id'];
}
mysql_select_db($database_cn, $cn);
$query_rs_dre = sprintf("SELECT * FROM dressing WHERE id = %s", GetSQLValueString($colname_rs_dre, "int"));
$rs_dre = mysql_query($query_rs_dre, $cn) or die(mysql_error());
$row_rs_dre = mysql_fetch_assoc($rs_dre);
$totalRows_rs_dre = mysql_num_rows($rs_dre);
?>
...
<table  class="body" id="customers" border="1" style="padding:5px;border:solid" cellspacing="-1"  align="center">
  ...
  <?php $total=0; do { ?>
    <tr style="alignment-adjust:central">
      <td colspan="2"><?php echo 'Dressing' ?></td>
      <td align="center"><?php 
	 echo $row_rs_dre['charge'];
	  
	  ?></td>
    </tr>
    <?php  $total = $total+$row_rs_dre['charge']; ?>
    <?php } while ($row_rs_dre = mysql_fetch_assoc($rs_dre)); ?>
  <!-- </td>
        </tr>
      </table> -->
  <tr>
    <td colspan="2" align="right">Total</td>
    <td colspan="1" align="right"><?php echo $total;?></td>
  </tr>
  <tr >
    <td colspan="4"> Recieved with thanks from <strong>
      <?php  echo $row_Recordset1['fname'];?>
      <?php '  '; echo $row_Recordset1['mname'];?>
      <?php '  '; echo $row_Recordset1['lname'];?>
      </strong> <br/>
      Rs <strong><?php echo $total; ?></strong> The Sum Of Rs <strong><?php echo convert_number_to_words($total); ?></strong></td>
  </tr>
  <tr>
