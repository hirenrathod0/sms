...
echo  $insertSQL = sprintf("INSERT INTO bill(total,pid,type,status) VALUES (%s, %s, %s, %s)",
  ...
  $P=$_GET['pid'];
  $insertGoTo = "instbill_opd.php?pid=".$P;
...
mysql_select_db($database_cn, $cn);
$query_fee1 = "SELECT * FROM other_chg";
...
if (isset($_GET['name'])) 
{
  $c1 = $_GET['name'];
mysql_select_db($database_cn, $cn);
$query_price1 = sprintf("SELECT * FROM other_chg WHERE name ='$c' ");
...
{	
//alert(abc);
	var string_url = "genbill.php?name="+name+"&pid="+pid;
...
?></td>
          ...
          <div class="col-lg-6">
            <div class="portlet portlet-default">
              <div class="portlet-heading">
                <div class="portlet-title">
                  <h4 style="float:left">Other Charges Bill Detail </h4>
                </div>
                <div class="portlet-widgets"> </div>
                <div class="clearfix"></div>
              </div>
              <div id="basicFormExample" class="panel-collapse collapse in">
                <div class="portlet-body" style="height:300px">
                  <form action="<?php echo $editFormAction; ?>" name="frm2" method="POST">
                    <table class="table">
                      <tr>
                        <td><div id="container"> <br />
                            <br />
...
do {  
?>
                            <option value="<?php echo $row_fee1['name']; ?>"  <?php if(isset($_GET['name'] )){  if($row_fee1['name']==$_GET['name']){  echo "selected"; } } ?> ><?php echo $row_fee1['name']?></option>
                            <?php
} while ($row_fee1 = mysql_fetch_assoc($fee1));
  $rows1 = mysql_num_rows($fee1);
  if($rows1 > 0) {
      mysql_data_seek($fee1, 0);
	  $row_fee1 = mysql_fetch_assoc($fee1);
  }
?>
                            </select>
                        ...
                        <td><?php if (isset($_GET['name'])) 
{
 if($totalRows_price1<=0)
               ...
               ?>
                          <div class="input-prepend input-append">
                            <input type="text" style="height:auto" class="form-control" readonly="readonly" name="price" value="<?php echo $row_price1['price']; ?>" />
                          </div>
                          <?php               
do {  
?>
                          <?php
} while ($row_price1 = mysql_fetch_assoc($price1));
							  }}
?></td>
                      </tr>
                      <tr>
                    ...
                    <input type="hidden" name="MM_insert" value="frm2" />
                  </form>
                </div>
              </div>
            </div>
                      ...
                      <tbody>
                        <?php $total=0; do { ?>
                          <tr>
                            ...
                            <td><a href="deltempbill.php?id=<?php echo $row_Recordset1['id']; ?>&pid=<?php echo $_GET['pid'];?>" class="btn-danger btn">Delete</a></td>
                          </tr>
