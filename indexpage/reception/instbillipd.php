...
echo $insertSQL = sprintf("INSERT INTO billhistry (name, price, total,pid, bid, type, status,numofd,ttl) VALUES (%s, %s, %s, %s, %s, %s, %s,%s,%s)",
					  ...
					  GetSQLValueString($row_billid['total'], "int"),
                       GetSQLValueString($pid, "int"),
						 ...
						 GetSQLValueString($row_billid['type'], "text"),
						  ...
						  GetSQLValueString($row_tempbill['total'], "text")
						  );
exit;
  mysql_select_db($database_cn, $cn);
...
$p=$_GET['pid'];
 $DELSQL = sprintf("DELETE FROM tempbill where pid='$p'",$cn);
  ...
  $DELGoTo = "printbillopd.php?pid=$p&bid=".$id;
                  ...
                  <?php do { ?>
                    <tr>
                      ...
                      <td><?php echo $row_tempbill['numofd']; ?>
                        <input type="text" id="price" name="numofd" value="<?php echo $row_tempbill['numofd']; ?>" /></td>
