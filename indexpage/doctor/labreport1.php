...
if ((isset($_GET['rep_id'])) && ($_GET['rep_id'] != "")) {
  ...
  $deleteSQL = sprintf("DELETE FROM doc_lab_report WHERE rep_id=%s",
                       GetSQLValueString($_GET['rep_id'], "int"));

  mysql_select_db($database_cn, $cn);
  $Result1 = mysql_query($deleteSQL, $cn) or die(mysql_error());

  $deleteGoTo = "labreport.php?pid=".$_GET['pid'];
                  ...
                  <tbody>
                    <?php do { ?>
                      <tr>
                        ...
                        <td><input type="checkbox" name="chk[]" class="checkbox-col" value="<?php echo $row_Recordset1['name']; ?>" onclick="getReportNames(this,<?php echo $_GET['pid']; ?>); " />
                          <script type="text/javascript">
							var selectedRepName = new Array();
							function getReportNames(repName,pid){
								if(repName.checked == true){
									window.location = 'selected_rep_name.php?repName='+ repName.value + '&pid=' + pid;
								}
							}
							</script>
                        </td>
    ...
    <div class="col-lg-6">
      <div class="row">
		...
		?>
                      ...
                      <tr valign="baseline">
                        <td nowrap="" align="center" colspan="2"><input type="submit" value="Submit" class="btn btn-green " name="submit"></td>
                      </tr>
                    </tbody>
                  </table>
                </form>
